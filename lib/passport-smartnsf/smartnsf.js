 ...
 *         User.findOne({ username: userdtails.username }, function (err, user) {
 *           done(err, user);
 *         });
 *       },
...
function SmartNSFStrategy(options, verify, extractor) {
    if (!verify || !extractor || !(options && options.smartNSFPath) || !(options && options.smartNSFHosth)) {
...
SmartNSFStrategy.prototype.authenticate = function(req) {
    var credentials = {};
    this._extractor.call(req, credentials);
    ...
    request.post({ url: this._smartnsfhost + "/names.nsf?login", form: credentials, jar: j, followAllRedirects: true }, function(err, httpResponse, body) {
        var cookies = j.getCookies(thi._smartnsfhost);
        if (err) {
            return this.error(err);
        }
        if (httpResponse.statusCode != 200) {
            return this.error("Authetication failed");
        }
        ...
        try {
            ...
            if (resp && resp.username) {
                ...
                this._verify.call(resp, verified);
            } else {
